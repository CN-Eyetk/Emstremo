...
from .modules.modules import EmoTrans, EmoTrans_wo_Emo, EmoTrans_wo_STRA
from .modules.modules import ContrastiveLoss, get_last_arg_where_equal
...
class BartEncoderLayer(nn.Module):
    def __init__(self, config: BartConfig, is_fuse: bool):
        super().__init__()
...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.n_strat = 8
        self.emotion_head = nn.Linear(config.d_model if not config.emo_use_cat_attn else config.d_model * 2 , self.n_emo_in)
    ...
    def forward(
        ...
        #if self.config.use_vad_labels and role_ids is not None:
        ...
        for encoder_layer in self.layers:
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                #situation_attention_mask = layer_outputs['situation_attention_mask']
        ...
        if role_ids is not None or vad_ids is not None:
            emotion_logits, emo_hidden = self.predict_emotion(
                hidden_states=hidden_states,
            ...
            strategy_logits, strategy_hidden, strategy_embs = self.predict_strategy(
                hidden_states=hidden_states,
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def init_strategy_embedding(self, weights):
        with torch.no_grad():
            self.model.encoder.strategy_embedding.weight[:weights.size(0),:] = weights
    def init_emotion_embedding(self, weights):
        with torch.no_grad():
            self.model.encoder.trans_mat.emotion_embedding.weight[:weights.size(0),:] = weights    
    @add_start_docstrings_to_model_forward(BART_INPUTS_DOCSTRING)
